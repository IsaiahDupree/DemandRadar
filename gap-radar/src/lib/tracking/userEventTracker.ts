/**
 * User Event Tracker - Client-Side SDK for GapRadar
 * ==================================================
 *
 * Lightweight client-side event tracking library for web applications.
 * Adapted from ACD User Event Tracker for GapRadar.
 *
 * Usage:
 * ```typescript
...
export class UserEventTracker {
  ...
  identify(userId: string, traits?: UserTraits): void {
    ...
    setStorageItem('_gapradar_uid', userId);
    if (traits) {
      setStorageItem('_gapradar_traits', JSON.stringify(traits));
    }
  ...
  reset(): void {
    ...
    setStorageItem('_gapradar_anon', this.anonymousId);
    if (typeof localStorage !== 'undefined') {
      localStorage.removeItem('_gapradar_uid');
      localStorage.removeItem('_gapradar_traits');
    }
    this.track('$reset', {});
  }
  ...
  private getOrCreateAnonymousId(): string {
    let id = getStorageItem('_gapradar_anon') || getCookie('_gapradar_anon');
    if (!id) {
      id = `anon_${generateId()}`;
      setStorageItem('_gapradar_anon', id);
      setCookie('_gapradar_anon', id, 365);
    }
    return id;
  }

  private getOrCreateSessionId(): string {
    const stored = getStorageItem('_gapradar_session');
    const lastActivity = parseInt(getStorageItem('_gapradar_last') || '0', 10);
    ...
    setStorageItem('_gapradar_session', id);
    return id;
  }
  ...
  private checkSessionTimeout(): void {
    ...
    if (Date.now() - this.lastActivity > timeout) {
      ...
      setStorageItem('_gapradar_session', this.sessionId);
      this.sessionStart = Date.now();
      this.scrollDepth = 0;
      this.trackSessionStart();
    }
  ...
  private updateActivity(): void {
    this.lastActivity = Date.now();
    setStorageItem('_gapradar_last', this.lastActivity.toString());
  }
